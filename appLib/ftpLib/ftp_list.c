...
int ftp_list(struct ftpconnection *connection, char *params, char showthem) {
	...
	if (response) {
		printf("ftp_list: bad response from ftp_open_data (%d)\n",response);
	...
	switch (response) {
		...
		case 426 :
		case 451 :
		case 450 : printf("Could not list directory\n");
			   return response;
			   break;
		case 421 : printf("Service unavailable\n");
			   ...
			   return response;
			   break;
		case 500 : printf("Server doesn't understand LIST\n");
			   return response;
			   break;
		case 501 : printf("Server doesn't understand LIST parameters\n");
			   return response;
			   break;
		case 502 : printf("Server doesn't understand LIST\n");
			   return response;
			   break;
		case 530 : printf("Not logged in\n");
			   return 0;
			   break;
		default  : printf("Unknown response to LIST: %d\n", response);
			   return response;
			   break;
	}
	printf("ftp_list: processing list\n");
...
int ftp_nlst(struct ftpconnection *connection, char *params, char *list[], int max_items) {
	...
	switch (response) {
		...
		case 421 : printf("Service unavailable\n");
			   ...
			   return response;
			   break;
		case 500 : printf("Server doesn't understand NLST\n");
			   connection->status = STATUS_IDLE;
			   return response;
			   break;
		case 501 : printf("Server doesn't understand NLST parameters\n");
			   connection->status = STATUS_IDLE;
			   return response;
			   break;
		case 502 : printf("Server doesn't understand NLST\n");
			   connection->status = STATUS_IDLE;
			   return response;
			   break;
		case 530 : printf("Not logged in\n");
			   ...
			   return 0;
			   break;
		case 550 : printf("No files found matching %s\n", params); 
			   connection->status = STATUS_IDLE;
			   return 0;
			   break;
		default  : printf("Unknown response to NLST: %d\n", response);
			   connection->status = STATUS_IDLE;
			   return response;
			   break;
	}
