	...
	if (!address) {
		...
		switch (response) {
			...
			case 421 : printf("Service unavailable\n");
				   ...
				   return 0;
				   break;
			case 500 : printf("Server doesn't understand PASV\n");
				   FREE(line);
				   return 0;
				   break;
			case 501 : printf("Server doesn't understand PASV parameters\n");
				   FREE(line);
				   return 0;
				   break;
			case 502 : printf("Server doesn't understand PASV\n");
				   FREE(line);
				   return 0;
				   break;
			case 530 : printf("Not logged in\n");
				   ...
				   return 0;
				   break;
			default  : printf("Unknown response to PASV: %d\n", response);
				   FREE(line);
				   return 0;
				   break;
		}
...
int ftp_open_data(struct ftpconnection *connection) {
	...
	switch (response) {
		...
		case 421 : printf("Service unavailable\n");
			   ...
			   return response;
			   break;
		case 500 : printf("Server doesn't understand PORT\n");
			   return response;
			   break;
		case 501 : printf("Server doesn't understand PORT parameters\n");
			   return response;
			   break;
		case 502 : printf("Server doesn't understand PORT\n");
			   return response;
			   break;
		case 530 : printf("Not logged in\n");
			   return response;
			   break;
		default  : printf("Unknown response to PORT: %d\n", response);
			   return response;
			   break;
	}
}
...
int ftp_wait_data(struct ftpconnection *connection) {
	...
	printf("Data connection opened to %s\n",inet_ntoa(servaddr.sin_addr));
	connection->status = STATUS_TRANSFER;
	return 1;
}
