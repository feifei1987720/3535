...
int tw2865_set_audio_samplerate(tw2865_audio_samplerate samplerate)
{
    ...
    for (i = 0; i < chips; i ++)
	{
        ...
        temp = tw2865_byte_read(i2c_addr, 0x70);
        temp &= ~0x08;
        temp |= 0x1<<3;
        tw2865_byte_write(i2c_addr,  0x70, temp);
        ...
        {
            tw2865_byte_write(i2c_addr, 0xE1, 0xc0);
            tw2865_byte_write(i2c_addr, 0xE2, 0xaa);
            tw2865_byte_write(i2c_addr, 0xE3, 0xaa);
        }
        else 
        {
            tw2865_byte_write(i2c_addr, 0xE1, 0xe0);
            tw2865_byte_write(i2c_addr, 0xE2, 0xbb);
            tw2865_byte_write(i2c_addr, 0xE3, 0xbb);
        }
    }
...
int tw2865_set_audio_format(tw2865_audio_format audio_format)
{
    ...
    for (i = 0; i < chips; i ++)
	{
        ...
        /* I2S format digital serial data sequence on ADATR pin */ 
        if (0 == audio_format.format)
        {
            /* Sequence of Audio to be Recorded */
            tw2865_byte_write(tw2865_i2c_addr[i], 0xD5, 0x98);
            tw2865_byte_write(tw2865_i2c_addr[i], 0xD6, 0xBA);
            tw2865_byte_write(tw2865_i2c_addr[i], 0xD7, 0x54);
            tw2865_byte_write(tw2865_i2c_addr[i], 0xD8, 0x76);
        }
        /* PCM format digital serial data sequence on ADATR pin */
        else
        {
            /* Sequence of Audio to be Recorded */
...
EXPORT_SYMBOL(tw2865_set_audio_bitwidth);
...
static void tw2865_audio_init(unsigned char chip_addr)
{
    ...
    tw2865_byte_write(chip_addr, 0x89, 0x01);/* AIN5MD=1, AFS384=0 */
}
