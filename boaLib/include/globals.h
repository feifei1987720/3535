...
struct request {                /* pending requests */
    ...
    char local_ip_addr[20]; /* for virtualhost */
    ...
    char buffer[20*1024 + 1]; /* generic I/O buffer */
    char request_uri[1024 + 1]; /* uri */
    char client_stream[20*1024]; /* data from client - fit or be hosed */
    char *cgi_env[50 + 4];             /* CGI environment */

#ifdef ACCEPT_ON
    char accept[0]; /* Accept: fields */
#endif
};
